...
public class PSOProcess implements PSOConstants {
		...
		while(t < MAX_ITERATION && err > ProblemSet.ERR_TOLERANCE) {
			...
			for(int i=0; i<SWARM_SIZE; i++) {
				...
				newVel[0] = (w * p.getVelocity().getPos()[0]) + 
							(r1 * C1) * (pBestLocation.get(i).getLoc()[0] - p.getPosition().getLoc()[0]) +
							(r2 * C2) * (gBestLocation.getLoc()[0] - p.getPosition().getLoc()[0]);
				newVel[1] = (w * p.getVelocity().getPos()[1]) + 
							(r1 * C1) * (pBestLocation.get(i).getLoc()[1] - p.getPosition().getLoc()[1]) +
							(r2 * C2) * (gBestLocation.getLoc()[1] - p.getPosition().getLoc()[1]);
				Velocity vel = new Velocity(newVel);
				p.setVelocity(vel);
				
				// step 4 - update location
				double[] newLoc = new double[PROBLEM_DIMENSION];
				newLoc[0] = p.getPosition().getLoc()[0] + newVel[0];
				newLoc[1] = p.getPosition().getLoc()[1] + newVel[1];
	...
	public void initializeSwarm() {
		...
		for(int i=0; i<SWARM_SIZE; i++) {
			p = new Particle();
			Representation_test rp=new Representation_test(2, 8);
			rp.init();
			rp.fitness_asses();
			rp.input();
			...
			loc[0]=rp.cost_array[0];
			loc[1]=rp.cost_array[1];
